select 
customers_name (select customers.name as customers_name from sales left outer join customers on sales.customer_id = customers.id),
genres_name (select genres.name as genres_name from sales left outer join genres on sales.genres_id = genres.id),
...
staff_system_name (select statuses.name as staff_system_name from sales left outer join statuses on sales.staff_system_id = statuses.id)
from sales;
